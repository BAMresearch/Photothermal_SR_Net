...
def build_gradDescent(prob, T):
    ...
    xhat_ = tf.maximum(By_, .0)
    layers.append(('GradDescent T=1', xhat_, None))
    # pdb.set_trace()
    for t in range(1, T):
        xhat_ = newtfidct(tf.multiply(S_, newtfdct(xhat_))) + By_
        xhat_ = tf.maximum(xhat_, .0)
        layers.append(('GradDescent T=' + str(t + 1), xhat_, None))
...
def build_LBFISTA(prob,T,initial_lambda=.1):
    ...
    for t in range(1,T):
        ...
        xhat_prev_ = xhat_ 
        lam_ = tf.Variable( initial_lambda,name='lam_{0}'.format(t) )
        xhat_ = newtfidct(tf.multiply(S_, newtfdct(z_))) + By_
        xhat_ = tf.maximum(xhat_, .0)
        xhat_ = blocksoft(xhat_, lam_)
...
def build_LBFastelastic_net(prob,T,initial_lambda=.1):
    ...
    tk = (1+np.sqrt(1+4*1**2))*2**(-1)
    z_ = xhat_
    layers.append( ('LBFelastic_net T=1',xhat_, (al0_, lam0_) ) )
    #pdb.set_trace()
    for t in range(1,T):
        t_prev = tk
        xhat_prev_ = xhat_ 
        ...
        tk = (1+np.sqrt(1+4*t_prev**2))*2**(-1)
        z_ = xhat_ + (t_prev-1)*(tk)**(-1)*(xhat_-xhat_prev_)
        layers.append( ('LBFelastic_net T='+str(t+1),xhat_,(al_, lam_)) )
