...
Net = 'LBFastElastic_Net' #possible: 'LBElastic_Net', 'LBFastElastic_Net', 'LBISTA', 'LBFISTA'
...
if Net == 'LBElastic_Net':
    ...
    # and do the training
    sess = train.do_training(training_stages,prob,'trained_networks/Untied_LBElastic_net_ThermoProbT'+str(T)+'Thermo6-Jan.npz', maxit = 5000)
if Net == 'LBFastElastic_Net':
    layers = network.build_LBFastelastic_net(prob, T=T, initial_lambda=0.1)
    training_stages = train.setup_training(layers, prob, trinit=1e-3, refinements=(.5,))
    sess = train.do_training(training_stages, prob,'trained_networks/Untied_LBFAST_net_mitMax_ThermoProbT' + str(T) + 'Thermo6-Jan.npz',maxit=5000)
if Net == 'LBISTA':
    layers = network.build_LBISTA(prob, T=T, initial_lambda=0.1)
    training_stages = train.setup_training(layers, prob, trinit=1e-3, refinements=(.5,))
    sess = train.do_training(training_stages, prob, 'trained_networks/Untied_LBISTA_mitMax_ThermoProbT' + str(T) + 'Thermo6-Jan.npz',maxit=5000)
if Net == 'LBFISTA':
    layers = network.build_LBFISTA(prob, T=T, initial_lambda=0.1)
    training_stages = train.setup_training(layers, prob, trinit=1e-3, refinements=(.5,))
    sess = train.do_training(training_stages, prob, 'trained_networks/Untied_LBFISTA_mitMax_ThermoProbT' + str(T) + 'Thermo6-Jan.npz',maxit=5000)
